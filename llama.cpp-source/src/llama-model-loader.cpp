...
void llama_model_loader::release_expert_memory() {
    LLAMA_LOG_DEBUG("%s: use_mmap=%d mappings=%zu weights_map=%zu\n", __func__, use_mmap, mappings.size(), weights_map.size());
    if (!use_mmap || mappings.empty()) {
        LLAMA_LOG_DEBUG("%s: skipping (no mmap or no mappings)\n", __func__);
        return;
    }

    // Identify expert weight tensors by name pattern.
    // Expert tensors contain "_exps" in their name (e.g. blk.0.ffn_gate_up_exps.weight).
    // We exclude router tensors (ffn_gate_inp) and per-tensor scales.
    ...
    for (const auto & it : weights_map) {
        ...
        // Calculate page-aligned range for this tensor
        ...
        if (start >= end) {
            // Tensor fits in a single page - release that whole page
