...
# --- Handle modes ---
if ($ask -ne "") {
    # Gemma 4 chat template format (the abliterated GGUF lacks an embedded
    # template, so we apply it manually — without it the model enters its
    # "setup" meta-mode instead of answering).
    ...
    try {
        $result = Invoke-RestMethod -Uri "${apiBase}/completion" -Method Post -ContentType "application/json" -Body $body -TimeoutSec 300
